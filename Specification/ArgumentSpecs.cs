...
namespace AutoBox.Specification
{
    ...
    public class ArgumentSpecs
    {
        ...
        {
            Box.Setup<ProductRepository>(x => x.Create(Arg.Varies<string>(), Arg.Varies<int>())).Caches(TimeSpan.FromSeconds(10));
        ...
        {
            Box.Setup<ProductRepository>(x => x.Create(Arg.Varies<string>(), 1)).Caches(TimeSpan.FromSeconds(10));
